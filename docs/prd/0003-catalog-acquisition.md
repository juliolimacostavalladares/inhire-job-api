...
- `CAT-FR-09`: o pipeline de coleta filtra e seleciona vagas dinamicamente a partir do Perfil Profissional do Candidato (`CandidateProfile` e `AutoApplyPolicy`), persistindo no Catálogo apenas vagas com match positivo com as skills, headline e targetRoles do usuário, sem termos ou tecnologias fixas no código.
...
- `CAT-AC-06`: ao coletar vagas de um Tenant, apenas as vagas compatíveis com os perfis de candidatos cadastrados no sistema são persistidas no Catálogo.
